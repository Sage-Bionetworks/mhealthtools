...
load_guanlab_model <- function() {
  weights <- readRDS("yfg_nn_weights.Rdata") 
...
#' t, x, y, z.
#' @param model A list of models to use for prediction.
...
guanlab_model <- function(sensor_data, model = load_guanlab_model) {
  if (is.function(model)) { # load weights from file
    model <- model()
  }
} 