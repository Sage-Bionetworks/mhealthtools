...
testthat::test_that('Generate GuanLab architechture',{
  ...
  testthat::expect_is(mhealthtools:::guanlab_nn_architecture(),'keras.engine.sequential.Sequential')
...
testthat::context('Extract features using GuanLab Model',{
  ...
  testthat::expect_is(mhealthtools::guanlab_model(sensor_data = datAccel), 'data.frame')
