####################################################
# File to test get_tremor_features.R of the mHealthTools package
...
test_that('Get accelerometer, gyroscope features',{
  ...
  expect_is(mhealthtools::get_tremor_features(accelerometer_data = datAccel, gyroscope_data = datGyro), 'list') 
  # Give both Accelerometer and Gyroscope data and expect a dataframe, with rest of the inputs being default
  expect_is(mhealthtools::get_tremor_features(accelerometer_data = datAccel, gyroscope_data = datGyro, gravity_data = datGravity), 'list') 
  # Similar test to previous one except also included gravity data
  
  expect_is(mhealthtools::get_tremor_features(accelerometer_data = datAccel, gyroscope_data = datGyro, funs = list(mean)), 'list')
  ...
  expect_is(mhealthtools::get_tremor_features(accelerometer_data = datAccel, gyroscope_data = datGyro, models = custom_model), 'list')
