...
# (throws error) get_walk_features
######################## *** NOTE *** ########################
...
test_that('Get accelerometer, gyroscope features',{
  ...
  expect_is(mhealthtools::get_walk_features(accelerometer_data = datAccel, gyroscope_data = datGyro, funs = list(mean)), 'data.frame')
  ...
  expect_is(mhealthtools::get_walk_features(accelerometer_data = datAccel, gyroscope_data = datGyro, models = custom_model, 'data.frame'))
