...
test_that('Function to extract heart rate per channel(R,G,B)',{
  # actual function in get_heartRate: getHeartRate
  ...
  expect_is(mhealthtools:::getHeartRate(datHR), 'data.frame') # Check if output is in correct format
  
  tempDat <- copy(datHR)
  tempDat <- tempDat %>% dplyr::rename('t' = 'timestamp') # Changed the column name of timestamp to t
  expect_equal(mhealthtools:::getHeartRate(tempDat), testTibble) # Error if timestamp column is missing
  
  tempDat <- copy(datHR)
  tempDat$timestamp <- rep(1, length(datHR$timestamp))
  expect_equal(mhealthtools:::getHeartRate(tempDat), testTibble) # Error if sampling rate cannot be calculated from timestamp
  ...
  expect_equal(mhealthtools:::getHeartRate(tempDat), testTibble) # Error if any channel red, green or blue is missing
