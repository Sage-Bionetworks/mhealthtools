...
testthat::test_that('Get accelerometer, gyroscope features',{
  ...
  testthat::expect_is(mhealthtools::get_rest_features(accelerometer_data = datAccel, gyroscope_data = datGyro), 'list') 
  # Give both Accelerometer and Gyroscope data and expect a dataframe, with rest of the inputs being default
  testthat::expect_is(mhealthtools::get_rest_features(accelerometer_data = datAccel, gyroscope_data = datGyro, gravity_data = datGravity), 'list') 
  # Similar test to previous one except also included gravity data
  
  testthat::expect_is(mhealthtools::get_rest_features(accelerometer_data = datAccel, gyroscope_data = datGyro, funs = list(mean)), 'list')
