...
### Get the formatted tapping, accelerometer and gyroscope data 
...
testthat::test_that('Wrapper to extract accelerometer features',{
  ...
  testthat::expect_is(mhealthtools:::accelerometer_features(
    sensor_data = datAccelTidy), 'list') 
...
testthat::test_that('Function to extract accelerometer features',{
  ...
  transformation <- mhealthtools:::transformation_window(window_length = 256,
                                                         overlap = 0.5) 
  # Required default transformation for the data
  funs <- mhealthtools:::default_kinematic_features(sampling_rate = 100) 
  # Required default feature extraction functions
  
  testthat::expect_is(
    mhealthtools:::accelerometer_features_(
...
testthat::test_that('Wrapper to extract gyroscope features',{
  ...
  testthat::expect_is(mhealthtools:::gyroscope_features(
    sensor_data = datGyro), 'list') 
  # Check if output is in correct format
...
testthat::test_that('Function to extract gyroscope features',{
  ...
  testthat::expect_is(
    mhealthtools:::gyroscope_features_(
      sensor_data = datAccel,
      transform = purrr::partial(
        mhealthtools:::transform_accelerometer_data,
        transformation = transformation,
...
testthat::test_that(
  'Function to transform accelometer data with given input parameters',{
    # actual function in sensors.R: transform_accelerometer_data
    
    testthat::expect_is(mhealthtools:::transform_accelerometer_data(
      sensor_data = datAccel), 'data.frame') 
    # Check if output is in correct format
  })

testthat::test_that(
  'Function to transform gyroscope data with given input parameters',{
    # actual function in sensors.R: transform_gyroscope_data
    
    testthat::expect_is(mhealthtools:::transform_gyroscope_data(
      sensor_data = datGyro), 'data.frame') 
    # Check if output is in correct format
  })

testthat::test_that(
  'Function to transform kinematic sensor with given input parameters',{
    # actual function in sensors.R: transform_kinematic_sensor_data
    
    testthat::expect_is(
      mhealthtools:::transform_kinematic_sensor_data(
        sensor_data = datAccel,
        transformation = NULL,
        window_length = 256,
        time_range = c(1,9),
...
testthat::test_that(
  'Function to initialize windowing transformation function with input
  parameters',{
    # actual function in sensors.R: transformation_window
    
    testthat::expect_is(
      mhealthtools:::transformation_window(
        window_length = 256,
        overlap = 0.5),
...
testthat::test_that(
  'Function to initialize IMF windowing transformation function with 
  input parameters',{
    # actual function in sensors.R: transformation_imf_window
    
    testthat::expect_is(
      mhealthtools:::transformation_imf_window(
        window_length = 256, 
        overlap = 0.5,
        max_imf = 4),
      'function') # Check if output is in correct format
...
testthat::test_that(
  'Function to initialize list of default kinematic feature extraction 
  functions',{
    # actual function in sensors.R: default_kinematic_features
    
    testthat::expect_is(
      mhealthtools:::default_kinematic_features(sampling_rate = 100),
      'list') # Check if output is in correct format  
  })
...
testthat::test_that('Preprocess sensor data',{
  ...
  testthat::expect_is(
    mhealthtools:::preprocess_sensor_data(sensor_data = datAccel,
                                          window_length = 256,
                                          sampling_rate = 100,
                                          frequency_range = c(1,25),
                                          time_range = c(1,9)),
...
testthat::test_that('Extract kinematic sensor features',{
  ...
  testthat::expect_is(
    mhealthtools:::kinematic_sensor_features(
      sensor_data = datAccel,
      transform = purrr::partial(
        mhealthtools:::transform_accelerometer_data,
        transformation = transformation,
...
testthat::test_that('Extract sensor features',{
  ...
  testthat::expect_is(
    mhealthtools:::sensor_features(
      sensor_data = transformed_accelerometer_data,
