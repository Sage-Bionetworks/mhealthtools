context("get_kinetic_tremor_features")

test_that("No arguments", {
  expect_equal(get_kinetic_tremor_features(),
...
test_that("Accelerometer data only", {
  features_accel_only <- get_kinetic_tremor_features(
...
test_that("Both accelerometer and gyroscope data", {
  features_both <- get_kinetic_tremor_features(
    accelerometer_data = mini_accelerometer_data,
...
test_that("Gravity data", {
  features_with_gravity <- get_kinetic_tremor_features(
    accelerometer_data = mini_accelerometer_data,
...
test_that("Passing a model", {
  ...
  features_with_model <- get_kinetic_tremor_features(
      accelerometer_data = mini_accelerometer_data,
      gyroscope_data = mini_gyroscope_data,
      window_length = 128,
      window_overlap = 0.2,
...
test_that("Input data contains NA", {
  expect_equal(is_error_dataframe(
    get_kinetic_tremor_features(accelerometer_data = NA)), T)
  expect_equal(is_error_dataframe(
    get_kinetic_tremor_features(gyroscope_data = NA)), T)
})