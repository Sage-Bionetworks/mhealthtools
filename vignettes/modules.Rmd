...
In this assay, the participant is asked to keep the phone in his/her pants' front pocket (if not in the waistband of the pants) and then walk in a straight line for some fixed amount of time (e.g. 30s). For someone with Parkinson's we expect to see symptoms of Dyskinesia and Bradykinesia. 
...
As you can see the accelerometer data in `walk_data` is missing the `t` column, but it is referenced as `walk_data$timestamp`. Let's get the acceleromter data into the `mhealthtools` tidy format before we extract features from it using `get_walk_features`. Similarly for the gyroscope data
...
In this assay, the participant is asked to hold the phone in his/her hand in their lap so that their resting tremor can be measured. For someone with Parkinson's we expect to see some effects of the characteristic pill rolling tremor spread across gyroscope and accelerometer measurements. 
...
As you can see the accelerometer data in `rest_tremor_data` is missing the `t` column, but it is referenced as `rest_tremor_data$timestamp`. Let's get the acceleromter data into the `mhealthtools` tidy format before we extract features from it using `get_balance_features`. Similarly for the gyroscope data
...
In this assay, the participant is asked to hold the phone in their hand, and extend their hands to shoulder length and then touch their nose with their finger, and keep this motion in a loop.
...
Let's get the acceleromter data into the `mhealthtools` tidy format before we extract features from it using `get_kinetic_tremor_features`. Similarly for the gyroscope data
...
In this assay, the participant is asked to tap the screen with their middle and pointing finger alternatingly as fast as they can for 30s (with one hand first, and if possible the other hand too). For people having Parkinson's they might not be able to tap as fast as a normal person, and also they might show higher fatigue (slower tap rate) at the end of the assay than a normal person.
...
In this assay, the participant is asked to place their finger over their smartphone camera and from the average pixel intensity across frames, we estimate the heartrate of the participant. 
